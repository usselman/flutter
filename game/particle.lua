...
function add_particles(_x,_y,_type,_dur,_col,_oldcol)
    local _p={}
    _p.x=_x
    _p.y=_y
    _p.t=_type
...
function spawntrail(_x,_y)
    ...
    if rnd()<0.9 then
        local _angle = rnd()
        local _dx = sin(_angle)*(player.w/8)*0.3
        local _dy = cos(_angle)*(player.h/8)*0.3

        add_particles(_x+_dx-player.w/8,_y+_dy+player.h/1.2,0,3+rnd(3),rnd(15),5)
...
function spawncircle(_x,_y)
    if rnd()>0.5 then
        ...
        add_particles(_x+4+_dx,_y+4+_dy,1,rnd(5),5,1)
    end
end
...
function draw_particles()
    for i=1,#particle do
        ...
        if _p.t==0 then
           ...
           pset(_p.x,_p.y+rnd(player.h/5),_p.col)
